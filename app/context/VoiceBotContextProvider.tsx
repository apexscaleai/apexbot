...
const defaultSleepTimeoutSeconds = 120;
...
export function VoiceBotContextProvider({ children }: Props) {
  ...
  // Disable sleep timer for now to fix the rapid cycling issue
  // useEffect(() => {
  //   const interval = setInterval(() => {
  //     if (currentStatusRef.current === VoiceBotStatus.LISTENING || currentStatusRef.current === VoiceBotStatus.SPEAKING) {
  ...
  // Disable automatic sleep triggering
  // useEffect(() => {
  //   if (state.sleepTimer > defaultSleepTimeoutSeconds) {
  //     startSleeping();
  //   }
  // }, [state.sleepTimer, startSleeping]);
  ...
  const contextValue = useMemo(
    ...
    [state, startListening, startSpeaking, startSleeping, toggleSleep, setAttachParamsToCopyUrl, displayOrder, addVoicebotMessage, addBehindTheScenesEvent],
  );
